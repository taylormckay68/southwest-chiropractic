...
import {CloseWrapper, CloseIcon, LinkWrapper, MainLink, MainLinkContainer, MenuWrapper, ServicesHeader, ServicesHeaderWrapper, ServicesMenuWrapper, SubService, SubServicesWrapper} from './styled-components/MobileMenu';
import {serviceData} from '../../utils';

export default function MobileMenu(props) {
    return(
        <MenuWrapper open={props.open} className="menu-wrapper">
            <CloseWrapper className="close-wrapper">
                <CloseIcon className="close-icon" onClick={() => props.toggleMenu('menu')}>
            ...
            <LinkWrapper className="link-wrapper">
                <MainLinkContainer className="main-link-container">
                    <MainLink className="main-link" to='/' onClick={() => props.toggleMenu('menu')}>Home</MainLink>    
                </MainLinkContainer>
                <ServicesMenuWrapper classname='services-menu-wrapper'>
                    <ServicesHeaderWrapper className='services-header-wrapper' onClick={() => props.toggleMenu('serviceMenu')}>
                        <ServicesHeader className='services-header'>Services</ServicesHeader>
                        {!props.serviceMenu ? 
                            <svg xmlns="http://www.w3.org/2000/svg" className='open-icon' width="24" height="24" viewBox="0 0 24 24"><path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 13h-5v5h-2v-5h-5v-2h5v-5h2v5h5v2z"/></svg>
                             : 
                             <svg xmlns="http://www.w3.org/2000/svg" className='close-icon' width="24" height="24" viewBox="0 0 24 24"><path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 13h-12v-2h12v2z"/></svg>
                             }
                    </ServicesHeaderWrapper>
                    <SubServicesWrapper className='sub-services-wrapper' open={props.serviceMenu}>
                        {serviceData.map((e,i) => {
                            return(
                                <SubService 
                                    ...
                                    key={i} 
                                    to={`/services/${e.link}`}
                                    onClick={() => props.toggleMenu('menu')}>{e.service}
                                </SubService>
                            )
                        })}
                    </SubServicesWrapper>
                ...
                <MainLinkContainer className='main-link-container'>
                    <MainLink className='main-link' to='/contact' onClick={() => props.toggleMenu('menu')}>Contact</MainLink>
                </MainLinkContainer>
            </LinkWrapper>
        </MenuWrapper>
    )
